f.pval <- function (dat, orth11, orth01, y.norm, rss00, df00)  {
	...
	p1 <- 1 - stats::pf(fstats, df1 = (df11 - df01), df2 = (n - df11))

	fstats <- ((rss00 - rss01)/(df01 - df00))/(rss01/(n - df01))
	p2 <- 1 - stats::pf(fstats, df1 = (df01 - df00), df2 = (n - df01))
...
#'
#' @inherit SmartSVA::smartsva
#' @export
hackedSVA <-  function(dat, mod, mod0 = NULL, n.sv, B = 100,
	...
	while (i < B && rho < 1 - epsilon) {
		...
		# Update spearman Rho.
		if (i > 1) {
			rho <- stats::cor(x=pprob, y=p.prev, use="pairwise.complete.obs",
								 method="spearman")
			p.prev <- pprob
		}else{
			p.prev <- pprob
	...
	sv <- uu[, 1:n.sv]
	retval <- list(sv = sv, rho = rho, iter = i, n.sv = n.sv, pprob.gam=pprob.gam, pprob.b=pprob.b)
	return(retval)
}
